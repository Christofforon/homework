...
def task_5(*args, **kwargs):
    numbers = sorted([arg for arg in args if isinstance(arg, int)])
    keys = sorted([key for key, value in kwargs.items() if isinstance(value, str)])
    return numbers, keys
    pass
# CODUL TĂU VINE MAI SUS:
...
def task_12(*args):
    for arg in args:
        ...
        if arg <= 3:
            return True
        if arg % 2 == 0 or arg % 3 == 0:
            return False
        i = 4
        while i * i <= arg:
            if arg % i == 0 or arg % (i + 2) == 0:
                return False
            i += 6
    return True
    pass
...
def task_13(string, *args):
    return all(sorted(arg) == sorted(string) for arg in args)
    pass
# CODUL TĂU VINE MAI SUS:
...
def task_20(*args):
    word_length_count = {}
    for arg in args:
        word_length_count[len(arg)] = word_length_count.get(len(arg), 0) + 1
